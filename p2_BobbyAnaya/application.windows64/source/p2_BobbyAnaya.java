...
public class p2_BobbyAnaya extends PApplet {
...
int centXSize = 200;
int centYSize = 325;
int baseColor1, dialBkgColor1, smallGaugeColor1, baseColor2, dialBkgColor2, smallGaugeColor2, baseColor3, dialBkgColor3, smallGaugeColor3, 
...
public void draw(){
  ...
  image(GasHashMarks,gasGaugeX-100,gasGaugeY-87.5f);
...
public boolean halfsecondcheck(){
  if(((millis()/1000) - time) >= 0.25f){
    time = millis()/1000;
    return true;
  }
  else{
...
public boolean overStart(int x, int y, int diameter){
  float disX = x - mouseX;
  float disY = y - mouseY;
...
public boolean overTriButton(float x1, float y1, float x2, float y2, float x3, float y3) {
  float px = mouseX;
  float py = mouseY;
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "p2_BobbyAnaya" };
    if (passedArgs != null) {
